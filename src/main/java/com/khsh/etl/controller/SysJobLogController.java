...
public class SysJobLogController extends CoBaseController { 
	...
	public Result query(@RequestBody(required=false)SysJobLogVO model) {
	...
	public Result delete(@RequestBody(required=true)SysJobLogVO model) {
		Result rs = new Result();
		try{
	...
	public Result add(@RequestBody(required=true)SysJobLogVO model) {
		Result rs = new Result();
		try{
	...
	public Result update(@RequestBody(required=true)SysJobLogVO model) {
		Result rs = new Result();
		try{
	...
	public Result queryByPage(@RequestBody(required=true)Param param, BindingResult bindResult) {
		...
		try{
			...
			SysJobLogVO model = toBean(param, new TypeReference<SysJobLogVO>(){});
			PageBean<SysJobLogModel> pageBean = mService.queryByPage(model, param.getPage().getPageNum(), param.getPage().getPageSize());
